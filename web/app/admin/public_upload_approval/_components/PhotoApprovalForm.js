// this is a client component that handles the frontend form logic for the image approval

"use client";

import { useFormStatus } from "react-dom";
import Loading from "@/app/_components/Loading";
import { deletePhoto } from "../page";

export default function PhotoApprovalForm({
  id,
  initial,
  CampusEnum,
  DiffEnum,
  ...
  return (
    <>
      ...
      <button type="submit" className="rounded bg-green-700 p-3 text-white">
        Save
      </button>
      <button
        onClick={handleDelete}
        className="mt-4 rounded bg-rose-600 p-3 text-white"
      >
        Delete
      </button>
      {pending && (
