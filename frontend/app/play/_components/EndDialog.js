...
export default function EndDialog({ gameState }) {
    ...
    const mapCenter = gameState.allLocsUsed.length > 0 ? calculateMapCenter(gameState.allLocsUsed) : [0, 0];
    ...
    return (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center">
            ...
            {/* Dialog container */}
            <dialog className="relative z-[1100] bg-white p-6 rounded-lg max-w-md w-full mx-4 text-center" open={true}>
                            ...
                            {gameState.allLocsUsed.map((loc, index) => (
                                <>
                                    <MapImageWrapper
                                        key={index}
                                        mapRef={map}
                                        guess={[gameState.allGuessesUsed[index][0], gameState.allGuessesUsed[index][1]]}  // Use each location's lat and lng
                                        setGuess={() => { }}
                                        actualLocation={[loc.lat, loc.lng]}
                                    />
                                    <Polyline
                                        positions={[[gameState.allGuessesUsed[index][0], gameState.allGuessesUsed[index][1]], [loc.lat, loc.lng]]}  // Line connecting all locations
                                        pathOptions={{
